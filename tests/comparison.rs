...
fn test_constant_array() -> Result<(), Error> {
    let array = ArrayBase::new(vec![2, 3], vec![0.; 6])?;
    assert!(!array.any()?);

    let array = ArrayBase::new(vec![2, 3], vec![1.; 6])?;
    assert!(array.all()?);
...
fn test_eq() -> Result<(), Error> {
    let zeros = ArrayBase::new(vec![2, 3], vec![0.; 6])?;
    let ones = ArrayBase::new(vec![2, 3], vec![1.; 6])?;
...
fn test_gt() -> Result<(), Error> {
    let zeros = ArrayBase::new(vec![4, 5, 7], vec![0.; 140])?;
    let ones = ArrayBase::new(vec![4, 5, 7], vec![1.; 140])?;
...
fn test_gte() -> Result<(), Error> {
    let zeros = ArrayBase::new(vec![5, 2], vec![0.; 10])?;
    let ones = ArrayBase::new(vec![5, 2], vec![1.; 10])?;
